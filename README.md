...
`ptra` creates multiple output files. This output is structured as follows:

1. A folder `unclustered-trajectories` with all trajectories found by `ptra` (before clustering). This folder contains the
   following files:

   1.1 a tab file with the found trajectories. The tab file contains two lines per trajectory. The first line lists the diagnoses
   ...
   Example:
    ...
    150 \tab 50
    ```
   1.2 a tab file with the found diagnosis pairs and their relative risk scores. There is a single line that list the diagnoses and the RR.

   Example:

   ```Cough \tab Dyspnea \tab 1.95```

   1.3 a gml file with all found trajectories as individual graphs. Useful for verifying outputs with few trajectories.

   1.4 a gml file with all found trajectories plotted as one (large) graph. Useful for verifying output, but may be too
   dense to view for large numbers of trajectories. In this case, clustering is recommended.

2. a folder `clustered-trajectories` with clustered trajectories --if `ptra` was requested to cluster its output
   (`--cluster` flag). This folder contains multiple subfolders:

   2.1 A folder `mclOutput` with intermediate clustering output generated by the MCL tool. This is normally not relevant
   to view for `ptra` users. It is useful for the `ptra` developers for debugging.
   2.2 A number of `clusters` folders containing clustered trajectories. A folder is generated for each chosen cluster
   granularity when calling `ptra`, cf. `--clusterGranularities` option. By default, 4 granuarities are tried, so 4
   `clusters` folders are created. Each `clusters` folder contains the following files:
   2.2.1 a csv file with cluster information. The header is: `PID,CID,TID,Age`. These represent the patient identifier, cluster
   identifier, trajectory identifier, and age of the patient at the time they completed the trajectory.
   2.2.2 a csv file with information to link the patient analysis identifier used in `ptra` back to the TriNetX identifier. The
   ...
   2.2.3 two graph modeling language (.gml) files with the clustered trajectories organised as a subgraph per cluster. gml files
   ...
   2.2.4 a DOT (.dot) file as alternative to the .gml files. DOT files can be visualised using [graphviz](https://graphviz.org/).
